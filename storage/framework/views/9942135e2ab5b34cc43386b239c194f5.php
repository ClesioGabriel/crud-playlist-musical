<?php $__env->startSection('title', 'Listagem das Playlists'); ?>
    ...
    <div class="flex flex-wrap gap-6">
        <?php $__empty_1 = true; $__currentLoopData = $playlists; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $playlist): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="w-[18%] bg-white dark:bg-gray-800 shadow-sm rounded-lg p-4 flex flex-col items-center">
                <a href="<?php echo e(route('playlists.show', $playlist->id)); ?>" class="block w-full">
                    <div class="w-[200px] h-[200px] flex items-center justify-center rounded-lg overflow-hidden mb-4 border border-gray-200 dark:border-gray-700 transition-transform transition duration-200 hover:scale-105 hover:shadow-lg">
                        <?php if($playlist->image): ?>
                            <img src="<?php echo e($playlist->image && file_exists(public_path('storage/' . $playlist->image)) ? asset('storage/' . $playlist->image) : asset('images/default-playlist.png')); ?>" alt="<?php echo e($playlist->name); ?>"
...
<?php echo $__env->make('admin.users.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/resources/views/playlists/index.blade.php ENDPATH**/ ?>